...
import { PasteImagePlugin } from "@/components/editor/plugins/paste-image-plugin"
...
export function Plugins({}) {
  const { editorWidth } = useDraftStore()
  const [floatingAnchorElem, setFloatingAnchorElem] =
    useState<HTMLDivElement | null>(null)
  const [isLinkEditMode, setIsLinkEditMode] = useState<boolean>(false)

  const widthClass = useMemo(() => WIDTH_CLASSES[editorWidth], [editorWidth])
  ...
  return (
    <div className="relative w-full">
      <ToolbarPlugin>
        {({ blockType }) => (
          <div className={`${widthClass} mx-auto px-4`}>
            <div
              className="sticky top-2 z-20 flex w-full items-center justify-start gap-1 overflow-x-auto border border-border bg-background/95 backdrop-blur px-4 py-2 rounded-xl shadow-sm"
            >
            {/* Undo/Redo Group */}
            <div className="flex items-center gap-0.5 bg-secondary dark:bg-zinc-800 rounded-lg p-1">
              <HistoryToolbarPlugin />
            </div>

            {/* Block Format */}
            <div className="flex items-center bg-secondary dark:bg-zinc-800 rounded-lg p-1">
              <BlockFormatDropDown />
            </div>
            ...
            {blockType === "code" ? (
              ...
              <>
                ...
                {/* More Formatting: Sub/Super + Link + Clear */}
                <div className="flex items-center gap-0.5 bg-secondary dark:bg-zinc-800 rounded-lg p-1">
      ...
      <div className="relative w-full overflow-auto">
        ...
        <RichTextPlugin
          contentEditable={
            <div className={`${widthClass} mx-auto relative transition-all duration-200`}>
              <div ref={onRef}>
                <ContentEditable
                  placeholder={placeholder}
                  className="ContentEditable__root relative block min-h-[50vh] px-12 py-6 focus:outline-none text-base leading-relaxed"
                  placeholderClassName="text-muted-foreground pointer-events-none absolute top-0 left-0 overflow-hidden px-12 py-6 text-ellipsis select-none"
                />
              </div>
            </div>
          }
